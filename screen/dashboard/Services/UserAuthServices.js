...
const UserAuthServices = {
  ...
  UserRegister: async (firstName,lastName,officerId, email, gender, phone, password ,datenam,departmentvalue) => {
        const config = {
          headers: { "Content-Type": `application/json` }
        }
        const body = {
  ...
  UserUpdatem: async (_id,token,firstName,lastName,officerId, email, gender, phone ,datenam,departmentvalue) => {
        const config = {
  ...
  GetMoodList: async () => {
        const config = {
          headers: { "Content-Type": `application/json` , "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlZDUzZmQxZmQyODY5MDUwMDk3MzY0NCIsImVtYWlsIjoiYWRtaW5AbWFpbC5jb20iLCJpYXQiOjE1OTEyODc2NzV9.8Ch0lThirNnlpMTyU72FQqy_Db2nGo1Bp5Z5D9nXChQ" }
        }
  ...
  GetMoodListUser: async (token) => {
        ...
        const data = await axios.post(`${BASE_URL}/mood/filterByUser`, body ,config)
        return data
  },
  GetChatUser: async (token,user_id) => {
        ...
        console.log(token);
        console.log(user_id);
        const data = await axios.get(`${BASE_URL}/user/getByDepartmentAndPeer/${user_id}`, config)
        return data
  },
  ...
  UploadImageUrl: async (token,media) => {
        ...
        const body = {
         ...
         console.log(body);
        const data = await axios.post(`${BASE_URL}/media/uploadImageUrl`, body ,config)
        return data
  },
  GetNotification: async (token) => {
        ...
        const data = await axios.get(`${BASE_URL}/notification/getByReceiver`, config)
        return data
  },
  GetComment: async (token,post_id) => {
        ...
        console.log(token);
        const data = await axios.get(`${BASE_URL}/comment/getByPost/${post_id}`, config)
        return data
  },
  GetHelp: async (token) => {
        const config = {
  ...
  GetActivityMood: async (token,startDate,endDate) => {
        ...
        console.log(body);
        const data = await axios.post(`${BASE_URL}/dailyMood/filterByUser`, body,config)
        return data
  },
