...
export default class EditpostScreen extends Component//<{}>
{
  ...
  componentDidMount() {
    ...
    var array = []
    console.log(this.props.navigation.state.params.item.media);
    this.props.navigation.state.params.item.media.forEach(function (item) {
      console.log("inner");
      console.log(item);
      RNFetchBlob.config({
        fileCache: true
        ...
        .then((res) => {
          ...
          if (status == 200) {
            ...
            base64Str.then((vale) => {
              console.log(vale);
              array.push('data:image/jpeg;base64,' + vale);
            });
          } else {
    ...
    this.setState({
      ...
      searchString: this.props.navigation.state.params.item.text,
      pickupimages: this.props.navigation.state.params.item.media,
      avatarSource: array
    })
  }
  ...
  async AddPostUser() {
    ...
    if (this.state.searchString != "") {
      ...
      try {
        const { data } = await UserAuthServices.UpdatePostuser(this.state.user.token, this.props.navigation.state.params.item._id, this.state.searchString, bse64)
  ...
  async DeletePostuser() {
    try {
      const { data } = await UserAuthServices.DeletePostuser(this.state.user.token, this.props.navigation.state.params.item._id)
      console.log(data.data);
      this.dropDownAlertRef.alertWithType('success', 'Success', "Post added success");
      this.props.navigation.pop(1)
      this.setState({
        ...
        <View style={styles.slide}>
          ...
          <View style={styles.navBar}>
            ...
            <View style={styles.leftContainer}>
              ...
              <Text style={{ fontSize: 20, color: "#000", marginTop: 10, fontWeight: 'bold', marginLeft: 10 }}>Update Post</Text>
            </View>
            ...
            <View style={styles.rightContainer}>
              {
                this.state.load ?
                  ...
                  <TouchableOpacity underlayColor='#FFFFF' onPress={() => this.AddPostUser()}>
                    <Text style={{ fontSize: 20, color: "#201F3E", marginTop: 10, fontWeight: 'bold', marginRight: 10 }}>Update</Text>
                  </TouchableOpacity>
              }
          ...
          <TouchableOpacity style={{ backgroundColor: 'red', padding: 5, bottom: 0, width: "100%", justifyContent: "center", position: "absolute" }} onPress={() => this.DeletePostuser()}>
            <Text style={{ color: "#fff", padding: 10, textAlign: "center" }}>Delete
                     </Text>
          </TouchableOpacity>
        </View>
      </>
