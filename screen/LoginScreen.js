...
export default class LoginScreen extends Component<{}>
{
  ...
  async loginuser(){
    if(this.state.email=="" || this.state.password==""){
        this.dropDownAlertRef.alertWithType('error', 'Error', "Email and Password not be empty");
      return
    }
    try {
      this.setState({load: !this.state.load})
      const { data } = await UserAuthServices.UserLogin(this.state.email, this.state.password)
      console.log(data);
       ...
       <StatusBar
         ...
         <View style={styles.slide}>
            <View style={{position:"absolute" , right:-15,top:30}}>
                <Image source={require("../assets/bgimage.png")} style={{width: 200,margin:10,resizeMode: 'contain',justifyContent: 'center',alignItems:'center'}} />
            </View>
           ...
           <Text style={styles.textlabale}>Please sign in to continue</Text>
             ...
             <View style={[styles.searchSection,{  elevation: this.state.ismail,
               shadowOffset: { width: this.state.ismail/2, height: this.state.ismail },
               shadowColor: '#F2F2F2',
               shadowOpacity: 1,
               }]}>
             <View
             ...
             />
              ...
              <View style={[styles.searchSection,{  elevation: this.state.ispasswoed,
                shadowOffset: { width: this.state.ispasswoed/2, height: this.state.ispasswoed },
                shadowColor: '#F2F2F2',
                shadowOpacity: 1,
                }]} >
              <View
          ...
          <TouchableHighlight underlayColor='#FFFFF'>
           ...
           <View style={styles.slide_login}>
           {
             ...
             <TouchableOpacity underlayColor='#FFFFF' onPress={()=>  this.loginuser()}>
               <Image source={require("../assets/login.png")} style={{width:(Dimensions.get('window').width)-40, resizeMode: 'contain',marginTop:30}} />
             </TouchableOpacity>
           }
          ...
          <View style={{marginRight:20}}>
            <Text style={{color: "#959da5",textAlign: 'right'}}>Don't Have an account ? <Text style={{color: "#000",}} onPress={()=>  this.props.navigation.navigate("SignupScreen") }>Sign Up</Text></Text>
          </View>
