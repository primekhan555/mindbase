  ...
  renderHorizontalLabels = config => {
    ...
    return [...Array(count === 1 ? 1 : count + 1).keys()].map((i, _) => {
      ...
      if(i==4){
        return (
          <Image
            source={require(`../../../assets/mood0.png`)}
            style={{width:20, height: 20, resizeMode: 'contain',marginTop:5 ,marginBottom:5,marginLeft:10}}
        ...
        return (
          <Image
            ...
            style={{width:20, height: 20, resizeMode: 'contain',marginTop:5 ,marginBottom:5,marginLeft:10,marginRight:5}}
            rotation={horizontalLabelRotation}
            origin={`${x}, ${y}`}
            key={Math.random()}
            x={x}
